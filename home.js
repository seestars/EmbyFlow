...
div.swiper {
	...
	--swiper-pagination-color: #6CCF65 !important;
...
.skeleton {
	background: linear-gradient(90deg, rgba(40, 40, 40, .7) 25%, rgba(60, 60, 60, .7) 50%, rgba(40, 40, 40, .7) 75%);
   ...
   <div class="mdl-spinner">
		...
		document.querySelectorAll(".view:not(.hide) .banner-item-play").forEach(async (dom, i) => {
			...
			dom.addEventListener("click", async () => {
				if (detail.Type == "Series") {
					let nextUp = await ApiClient.getNextUpEpisodes({ SeriesId: detail.Id, UserId: ApiClient.getCurrentUserId() });
					if (nextUp.Items.length > 0) {
						playbackmanager.play({
							ids: [nextUp.Items[0].Id],
							serverId: nextUp.Items[0].ServerId,
							items: [nextUp.Items[0]]
						});
						return;
					}
				}
	...
	static async getLibItems(refreshFlag) {
		if (!refreshFlag && localStorage.getItem("CACHE|getLibItems" + ApiClient.getCurrentUserId() + "-" + ApiClient.serverId())) {
			const datetime = await new Promise((resolve) => require(["datetime"], resolve));
...
`;
		...
		let cachedData = localStorage.getItem("CACHE|getLibItems" + ApiClient.getCurrentUserId() + "-" + ApiClient.serverId());
		
		// Check if cache is expired based on date
		if (cachedData) {
			const datetime = await new Promise((resolve) => require(["datetime"], resolve));
			let currentDate = datetime.toLocaleDateString(new Date(), { dateStyle: "short" });
			if (localStorage.getItem('CACHE|Movies-Date') !== currentDate) {
				cachedData = null; // Force refresh
			}
		}

		let useSkeleton = !cachedData;
		...
		if (cachedData) {
			...
			let libdata = await ApiClient.getUserViews({}, ApiClient.getCurrentUserId());
			let libdataitem = this.user.Policy.EnableAllFolders ? libdata.Items : libdata.Items.filter(m => this.user.Policy.EnabledFolders.includes(m.Guid));
			this.Alldata = libdataitem.map(item => ({
				Id: item.Id, Name: item.Name, ImageTags: item.ImageTags, CollectionType: item.CollectionType, data: []
			}));
			// Start async loading
			this.lazyLoadSlides(element);
		}
		...
		for (let i = 0; i < this.Alldata.length; ++i) {
			...
			if (!useSkeleton && datas.data && datas.data.length > 0) {
...
<div class="swiper-slide skeleton-slide">
   <div class="banner-cover skeleton"></div>
   <div class="custom padded-left">
      <div class="title skeleton skeleton-title"></div>
      <div class="subtitle skeleton skeleton-text" style="width: 40%"></div>
      <div class="text">
...
`;
	...
	static async lazyLoadSlides(element) {
		for (let i = 0; i < this.Alldata.length; ++i) {
			let libitem = this.Alldata[i];
			...
			if (dataquery.Items.length === 0) {
				dataquery = await this.getItems({
					ImageTypes: "Backdrop", EnableImageTypes: "Primary,Backdrop,Banner,Logo",
					IncludeItemTypes: "Movie,Series", SortBy: "DateCreated",
					Recursive: true, ImageTypeLimit: 1, Limit: this.itemQuery.Limit, Fields: "Taglines,Overview",
					EnableUserData: true, EnableTotalRecordCount: false
				});
			}
			...
			libitem.data = dataquery.Items;
			
			let allbackdropHtml = '';
			let hvclass = libitem.CollectionType === "homevideos" ? "homevideos" : "";
			let isMobile = window.innerWidth < 800;

			for (let j = 0; j < libitem.data.length; ++j) {
				let detail = libitem.data[j];
				let ImageUrl;
				if (libitem.CollectionType === "homevideos") {
					ImageUrl = await this.getImageUrl(detail, this.coverOptions);
				} else {
					if (isMobile) {
						ImageUrl = await this.getImageUrl(detail, this.coverOptions);
				...
				const backdropHtml = `
<div class="swiper-slide" id="${detail.Id}">
   <img id="${detail.Id}" data-parentid="${libitem.Id}"  class="banner-cover ${hvclass}" draggable="false" loading="${j === 0 ? 'eager' : 'lazy'}" decoding="async" src="${ImageUrl}" />
   `+ ((detail.ImageTags && detail.ImageTags.Logo) ? `
   <img draggable="false" loading="${j === 0 ? 'eager' : 'lazy'}" decoding="async" class="banner-logo padded-right" src="${await this.getImageUrl(detail, this.logoOptions)}">` : ``) + `
   <div class="banner-mask"></div>
   <div class="custom padded-left">
      ...
      <div class="morebutton" data-focusabletype="nearest">
         ...
         <button id="${detail.Id}" data-parentid="${libitem.Id}" class="banner-item-play" tabindex="-1">PLAY</button>
		 <button tabindex="-1" onclick="Emby.Page.showItem('${detail.Id}')">MORE</button>
		 <br> </br>
	  </div>
   </div>
...
`);
			...
			let swiperWrapper = element.querySelector(`.mySwiper${i} .swiper-wrapper`);
			if (swiperWrapper) {
				swiperWrapper.innerHTML = allbackdropHtml;
				let swiperInstance = element.querySelector(`.mySwiper${i}`).swiper;
				if (swiperInstance) {
					swiperInstance.update();
					swiperInstance.slideTo(0);
                    if (this.swiper2 && this.swiper2.activeIndex === i) {
    ...
    4000: { slidesPerView: 10 } // ✅ Added breakpoint for ultra-wide screens
				...
				breakpoint: function (swiper) {
					...
					if (swiper.params.slidesPerView === 1) {
						...
						document.querySelectorAll(".banner-cover").forEach(async function (element, index) {
							...
							if (src1 && src1.includes("Backdrop")) {
								let id = element.getAttribute("id");
								let parentid = element.getAttribute("data-parentid");
								let parentItem = this.Alldata.filter(m => m.Id === parentid)[0];
								if (parentItem && parentItem.data) {
									let item = parentItem.data.filter(m => m.Id === id)[0];
									if (item) {
						...
						document.querySelectorAll(".banner-cover").forEach(async function (element, index) {
							let src1 = element.getAttribute("src");
							if (src1 && src1.includes("Primary")) {
								let id = element.getAttribute("id");
								let parentid = element.getAttribute("data-parentid");
								let items = this.Alldata.filter(m => m.Id === parentid)[0];
								if (items && items.CollectionType !== "homevideos" && items.data) {
		...
		this.swiper2 = new Swiper(e.querySelector(".mySwiper-main"), {
			...
			navigation: {
				nextEl: ".mySwiper-main > .swiper-button-next",
				prevEl: ".mySwiper-main > .swiper-button-prev",
			},
			on: {
				progress: function (swiper) {
					for (var i = 0; i < swiper.slides.length; i++) {
